...
class CavitySelfKerr(Experiment):
    """Cavity spectroscopy"""
    ...
    def sequence(self):
        ...
        # Displacement on cavity 
        self.cavity.play(self.cavity_drive, ampx = self.disp_ampx)
        
        # Wait for phase accumulation
        qua.wait(self.time_delay, self.cavity)
        
        # Displace cavity back with virtual detuning
        self.cavity.play(self.cavity_drive, ampx = self.disp_ampx, phase=self.phase)
        qua.align(self.cavity, self.qubit)
        
        # Try to flip qubit, flippable if cav goes back to vacuum
