...
class CavitySWAP1D_len_amp_Cavm(Experiment):
    ...
    def sequence(self):
        ...
        qua.align()
        self.cavity_m.play(self.cavity_m_drive, ampx=self.cav_m_ampx)
        qua.align(self.cavity_m, self.snail)
        self.snail.play(self.snail_pulse, duration=self.length_snail, ampx= 0.05) # self.snail_ampx
...
if __name__ == "__main__":
    ...
    DEL = Sweep(name="length_snail", start=4, stop=2000, step=16, dtype=int)
    SNAIL_AMPX = Sweep(
        name="cav_m_ampx",
        points=[
           0.0,1.0
