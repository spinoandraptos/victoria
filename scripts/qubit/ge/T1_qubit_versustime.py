...
class QubitT1_versus_time(Experiment):
    """Cavity spectroscopy"""
...
if __name__ == "__main__":
    ...
    DEL = Sweep(name="time_delay", start=16, stop=8_000, step=100, dtype=int)
    sweeps = [N, DEL]
    ...
    flux_values = 10000
    for index_f in range((flux_values)): 
        # with Stage(configpath=MODES_CONFIG, remote=True) as stage:
        #     (yoko1,) = stage.get("yoko1")
        #     yoko_target = flux_values[index_f]
            # yoko1.ramp(yoko_target, step=0.1e-3)
        expt = QubitT1_versus_time(FOLDER, modes, pulses, sweeps, datasets, **parameters)
        expt.run()
        # expt.run(simulate=True)
        time.sleep(5)  # Sleeps for 1 second; adjust as needed
